...
:: Makes sure that the copy script can successfully copy files to multiple target directories using the rp command
...
IF %ERRORLEVEL%==0 (
    ...
    SET success1=false
    IF EXIST "%target1%" (
        ECHO And files were successfully copied by replacement to the 1st target directory^^!
        SET success1=true
    ) ELSE (
        ECHO But files were not copied by replacement to "%targetDir1%"^^!
    )
    
    SET success2=false
    IF EXIST "%targetDir2%" (
        ECHO And files were successfully copied by replacement to the 2nd target directory^^!
        SET success2=true
    ) ELSE (
        ECHO But files were not copied by replacement to "%targetDir2%"^^!
    )
