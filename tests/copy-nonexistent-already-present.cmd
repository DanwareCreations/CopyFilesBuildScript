...
:: Makes sure that the copy script succeeds (with a warning) when the targets file tries to copy a nonexistent file that is already present in the target directory
...
:: Assert
IF %ERRORLEVEL%==0 (
    ECHO copyFiles correctly succeeded, even though script tried to copy a nonexistent file
    SET exitCode=0
) ELSE (
    ECHO copyFiles should have succeeded even though script tried to copy a nonexistent file, but it didn't^^!
    SET exitCode=1
)
